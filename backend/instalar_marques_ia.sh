#!/usr/bin/env bash
# ============================================================
#  INSTALADOR AUTOMÁTICO — MARQUES IA (Backend)
#  Cria todos os arquivos, o ambiente virtual e instala tudo.
#  Uso:  bash instalar_marques_ia.sh
# ============================================================
set -e

echo "==> Criando pastas..."
mkdir -p app/core app/ai/providers app/db app/memory tests

echo "==> Criando arquivos do projeto..."
cat > 'requirements.txt' << 'MARQUES_IA_EOF'
# ============================================================================
...
# O asyncpg só é necessário quando você usar PostgreSQL. Ele exige
# compilação e pode travar em alguns Macs. Para desenvolvimento local com
# SQLite, NÃO é preciso. Descomente a linha abaixo apenas em produção:
# asyncpg==0.30.0
...
cat > '.env.example' << 'MARQUES_IA_EOF'
# ============================================================================
...
cat > 'app/__init__.py' << 'MARQUES_IA_EOF'
...
cat > 'app/core/__init__.py' << 'MARQUES_IA_EOF'
...
cat > 'app/core/config.py' << 'MARQUES_IA_EOF'
...
cat > 'app/core/logging.py' << 'MARQUES_IA_EOF'
...
cat > 'app/core/exceptions.py' << 'MARQUES_IA_EOF'
...
cat > 'app/ai/__init__.py' << 'MARQUES_IA_EOF'
...
cat > 'app/ai/schemas.py' << 'MARQUES_IA_EOF'
...
cat > 'app/ai/base.py' << 'MARQUES_IA_EOF'
...
cat > 'app/ai/providers/__init__.py' << 'MARQUES_IA_EOF'
...
cat > 'app/ai/providers/claude_provider.py' << 'MARQUES_IA_EOF'
...
cat > 'app/ai/providers/openai_provider.py' << 'MARQUES_IA_EOF'
...
cat > 'app/db/__init__.py' << 'MARQUES_IA_EOF'
...
cat > 'app/db/base.py' << 'MARQUES_IA_EOF'
...
cat > 'app/db/models.py' << 'MARQUES_IA_EOF'
...
cat > 'app/memory/__init__.py' << 'MARQUES_IA_EOF'
...
cat > 'app/memory/repository.py' << 'MARQUES_IA_EOF'
...
cat > 'app/memory/service.py' << 'MARQUES_IA_EOF'
...
cat > 'app/main.py' << 'MARQUES_IA_EOF'
...
cat > 'tests/__init__.py' << 'MARQUES_IA_EOF'
"""Pacote de testes automatizados."""
MARQUES_IA_EOF

cat > 'tests/test_ai_manager.py' << 'MARQUES_IA_EOF'
...
cat > 'tests/test_memory.py' << 'MARQUES_IA_EOF'
...
echo ""
echo "======================================================"
echo " TUDO PRONTO! Faltam so 2 passos:"
echo ""
echo " 1) Coloque sua chave da Anthropic no arquivo .env:"
echo "    Abra o arquivo .env e edite a linha ANTHROPIC_API_KEY"
echo ""
echo " 2) Rode a aplicacao com estes dois comandos:"
